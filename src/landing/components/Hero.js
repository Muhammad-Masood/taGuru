import styles from "../style";
import { discount, robot, hero } from "../assets";
import GetStarted from "./GetStarted";
import { TypeAnimation } from "react-type-animation";

const Hero = () => {
  return (
    <section style={{ marginTop: "-70px" }} id="home" className={`flex md:flex-row flex-col ${styles.paddingY}`}>
      <div className={`flex-1 ${styles.flexStart} flex-col xl:px-0 sm:px-16 px-6`}>
        <div className="flex flex-row items-center py-[6px] px-4  rounded-[10px] mb-2">
          <img src={discount} alt="discount" className="w-[32px] h-[32px]" />
          <p className={`${styles.paragraph} ml-2`}>
            <TypeAnimation
              sequence={[
                "Welcome",
                500,
                "Welcome To", //  Continuing previous Text
                500,
                "Welcome To TA",
                500,
                "Welcome To TA GURU",
                500,
                "",
                500,
              ]}
              style={{ fontSize: "1em", color: "white" }}
              repeat={Infinity}
            />
            {/* <span className="text-white">Welcome To TA GURU </span> */}
            <span className="text-white"></span>
          </p>
        </div>

        <div className="flex flex-row justify-between items-center w-full">
          <h1 className="flex-1 font-poppins font-semibold ss:text-[72px] text-[52px] text-white ss:leading-[100.8px] leading-[75px]">
            <span className="text-gradient">Where Talent</span>{" "}
          </h1>
          {/* <div className="ss:flex hidden md:mr-4 mr-0">
            <GetStarted />
          </div> */}
        </div>

        <h1 className="font-poppins font-semibold ss:text-[68px] text-[52px] text-white ss:leading-[100.8px] leading-[75px] w-full">
          Meets Technology
        </h1>
        <p style={{ color: "white" }} className={`${styles.paragraph} max-w-[470px] mt-5`}>
          Discover TA GURU, the innovative job search and hiring platform that leverages blockchain technology to
          revolutionize the recruitment process. Join us now to experience the power of our blockchain-based recruitment
          system.
        </p>
      </div>

      <div className={`flex-1 flex ${styles.flexCenter} md:my-0 my-10 relative`}>
        <img src={hero} alt="billing" className="w-[1000px] h-[100%] relative z-[5] mr-20" />

        {/* gradient start */}
        <div className="absolute z-[0] w-[40%] h-[35%] top-0 pink__gradient" />
        <div className="absolute z-[1] w-[80%] h-[80%] rounded-full white__gradient bottom-40" />
        <div className="absolute z-[0] w-[50%] h-[50%] right-20 bottom-20 blue__gradient" />
        {/* gradient end */}
      </div>

      {/* <div className={`ss:hidden ${styles.flexCenter}`}>
        <GetStarted />
      </div> */}
    </section>
  );
};

export default Hero;
