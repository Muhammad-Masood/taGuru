import {
  people01,
  people02,
  people03,
  facebook,
  instagram,
  linkedin,
  twitter,
  airbnb,
  binance,
  coinbase,
  dropbox,
  send,
  shield,
  star,
} from "../assets";

export const navLinks = [
  {
    id: "home",
    title: "Home",
  },
  {
    id: "aboutus",
    title: "Aboutus",
  },
];

export const features = [
  {
    id: "feature-1",
    icon: star,
    title: "Tokens Rewards",
    content:
      "TA GURU benefits its users with TAG tokens.TAG tokens can be used as currency for purchasing goods/services and potentially increasing revenue.",
  },
  {
    id: "feature-2",
    icon: shield,
    title: "Instant and Accurate",
    content:
      "Our system guarantees pertinent data to create an accurate screening process to match the right candidate with the right job in less time.",
  },
  {
    id: "feature-3",
    icon: send,
    title: "Save Time & Money",
    content:
      "Our decentralized system enables tracking and storage of candidate data, increasing efficiency and reducing recruitment costs.",
  },
];

export const feedback = [
  {
    id: "feedback-1",
    content: "Using TA Guru made my job search much easier and more efficient than other platforms I've used before.",
    name: "John Smith",
    title: "",
    img: people01,
  },
  {
    id: "feedback-2",
    content:
      "The AI-assisted candidate screening on TA Guru allowed us to save time and resources while still finding the best fit for our company",
    name: "Acme Inc",
    title: "",
    img: people02,
  },
  {
    id: "feedback-3",
    content: "I appreciate the transparency and fairness of the recruitment process on TA Guru, it's refreshing.",
    name: "Emily Jones",
    title: "",
    img: people03,
  },
];

export const stats = [
  {
    id: "stats-1",
    title: "User Active",
    value: "3800+",
  },
  {
    id: "stats-2",
    title: "Trusted by Company",
    value: "230+",
  },
  {
    id: "stats-3",
    title: "Transaction",
    value: "$230M+",
  },
];

export const footerLinks = [
  {
    title: "Useful Links",
    links: [
      {
        name: "Manifesto",
        link: "https://muhammads-organization-8.gitbook.io/untitled-1/",
      },
      {
        name: "How it Works",
        link: "https://www.hoobank.com/how-it-works/",
      },
      {
        name: "Create",
        link: "https://www.hoobank.com/create/",
      },
      {
        name: "Explore",
        link: "https://www.hoobank.com/explore/",
      },
      {
        name: "Terms & Services",
        link: "https://www.hoobank.com/terms-and-services/",
      },
    ],
  },
  {
    title: "Community",
    links: [
      {
        name: "Help Center",
        link: "https://www.hoobank.com/help-center/",
      },
      {
        name: "Partners",
        link: "https://www.hoobank.com/partners/",
      },
      {
        name: "Suggestions",
        link: "https://www.hoobank.com/suggestions/",
      },
      {
        name: "Blog",
        link: "https://www.hoobank.com/blog/",
      },
      {
        name: "Newsletters",
        link: "https://www.hoobank.com/newsletters/",
      },
    ],
  },
  {
    title: "Partner",
    links: [
      {
        name: "Our Partner",
        link: "https://www.hoobank.com/our-partner/",
      },
      {
        name: "Become a Partner",
        link: "https://www.hoobank.com/become-a-partner/",
      },
    ],
  },
];

export const socialMedia = [
  {
    id: "social-media-1",
    icon: instagram,
    link: "https://www.instagram.com/",
  },
  {
    id: "social-media-2",
    icon: facebook,
    link: "https://www.facebook.com/",
  },
  {
    id: "social-media-3",
    icon: twitter,
    link: "https://www.twitter.com/",
  },
  {
    id: "social-media-4",
    icon: linkedin,
    link: "https://www.linkedin.com/",
  },
];

export const clients = [
  {
    id: "client-1",
    logo: airbnb,
  },
  {
    id: "client-2",
    logo: binance,
  },
  {
    id: "client-3",
    logo: coinbase,
  },
  {
    id: "client-4",
    logo: dropbox,
  },
];
